...
ws.on('connection', function(socket) {
	...
	socket.send(JSON.stringify({'action': 'canvasInfo', 'width': CANVAS_WIDTH, 'height': CANVAS_HEIGHT}));
		...
		switch (action) {
			...
			case 'paint':
				...
				if (x >= 0 && y >= 0 && x < CANVAS_WIDTH && y < CANVAS_HEIGHT) {
					...
					// TODO: check if the user hasn't made a request too early
					
					pixels[x][y]['color'] = color;
					pixels[x][y]['date'] = Date.now();
				
					var sendData = JSON.stringify({'action': 'updatePixel', 'data': pixels[x][y]});
			...
			case 'getPixel':
				...
				socket.send(JSON.stringify({'action': 'updatePixel', 'data': pixels[x][y]}));
				break;
